...
const manifest = JSON.parse(await readFile(new URL('../publishing/2026-09-01-manifest.json', import.meta.url)));
...
for (const route of routes) {
  ...
  assert.match(html, /September 1, 2026/, `${route} must show the visible date`);
  assert.match(html, /2026-09-01/, `${route} must contain the structured publication date`);
  assert.ok(html.includes(`rel="canonical" href="${canonical}"`), `${route} must be self-canonical`);
  assert.ok(html.includes(`"datePublished":"2026-09-01"`), `${route} must expose datePublished`);
...
console.log('PASS: exactly 12 September 1 Blog and 5 Research routes; HTTP 200, unique rendered hashes, visible/structured dates, self-canonicals, indexes, and sitemap verified');
