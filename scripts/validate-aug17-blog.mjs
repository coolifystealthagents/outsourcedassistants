...
const block = data.match(/const aug17ReplacementPosts = \[(.*?)\] as const;/s)?.[1] ?? '';
const records = [...block.matchAll(/\{ slug: '([^']+)'[^\n]+published: '2026-08-17' \}/g)].map((m) => m[1]);
if (records.length !== 22 || new Set(records).size !== 22) throw new Error(`Expected 22 unique August 17 records, found ${records.length}`);
const details = fs.readFileSync(path.join(root, 'app/aug17-blog.ts'), 'utf8');
for (const slug of records) {
  ...
  if (!/<time[^>]+dateTime="2026-08-17"[^>]*>August 17, 2026<\/time>/.test(html)) throw new Error(`Visible date missing: ${slug}`);
  if (!html.includes('datePublished') || !html.includes('2026-08-17')) throw new Error(`Structured date missing: ${slug}`);
...
if (positions.some((p) => p < 0) || positions.some((p, i) => i > 0 && p < positions[i - 1])) throw new Error('August 17 batch is not ordered in complete blog pagination');
const sitemap = fs.readFileSync(path.join(root, '.next/server/app/sitemap.xml.body'), 'utf8');
if (records.some((slug) => !sitemap.includes(`/blog/${slug}`))) throw new Error('August 17 route missing from sitemap');
console.log('PASS: 22 August 17 Blog records; direct bindings, visible and structured dates, canonicals, ordering, sitemap, photo, and zero forms verified');
